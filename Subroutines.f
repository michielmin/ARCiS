...
3	read(20,*,err=3) x1
	if(x0.gt.x1) then
		close(unit=20)
		call regridlog_inverse(input,grid,y,n)
		return
	else
		rewind(20)
	endif
...
c-----------------------------------------------------------------------
	...
	subroutine regridlog_inverse(input,grid,y,n)
	IMPLICIT NONE
	integer i,j,n
	real*8 grid(n),y(n),x0,y0,x1,y1
	real*8 lx0,ly0,lx1,ly1,lx,ly
...
100	read(20,*,end=102) x1,y1
	if(y1.le.0d0) y1=y0*1d-50
101	if(grid(i).ge.x1.and.grid(i).lt.x0) then
		y(i)=10d0**(log10(y1)+(log10(grid(i)/x1))*(log10(y0/y1))/(log10(x0/x1)))
		i=i-1
		if(i.lt.1) goto 102
		goto 101
...
c-----------------------------------------------------------------------
	...
	subroutine regridUV(input,grid,y,n)
	IMPLICIT NONE
	integer i,j,n
	real*8 grid(n),y(n),x0,y0,x1,y1
	real*8 lx0,ly0,lx1,ly1,lx,ly
