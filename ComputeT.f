...
c=========================================================================================
	...
	T1=T
	do ir=2,nr-1
		if(.not.Convec(ir).or.(.not.Convec(ir+1).and..not.Convec(ir-1))) T(ir)=(T1(ir-1)*T1(ir+1)*T1(ir)**2)**0.25
	enddo
	...
	real*8 exptau(nr),dtau(nr),x1,y10(nr),y01(nr),ex,invx
	...
	do ir=1,nr
		...
		if (x1.lt.1d-4) then
			...
			ex = exptau(ir)
			invx = 1d0/x1
			y10(ir) = 1d0 - (1d0 - ex)*invx
			y01(ir) = (1d0 - ex)*invx - ex
		endif
	enddo
	...
	do jr=1,nr
		...
		if(jr.lt.nr) then
			ir=jr
			Ip(jr+1)=Ip(jr)*exptau(ir)+y01(ir)
			if(jr.lt.nr-1) then
				do ir=jr+2,nr
