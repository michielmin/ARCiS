	...
	do while(.not.key%last)
		select case(key%key1)
			...
			case("lamstep","lam_step")
				if(key%nr1.eq.0) key%nr1=1
				if(key%nr2.eq.0) key%nr2=1
				if(key%nr1.gt.nStep) nStep=key%nr1
			case("cia")
				if(key%key2.eq.' ') then
...
c			read(key%value,*) nTpoints
		...
		case("fit_albedo_sigma_slope")
			read(key%value,*) fit_albedo_sigma_slope
		case("fit_albedo_l")
		...
		case("fit_albedo_remove_lin")
			read(key%value,*) fit_albedo_remove_lin
		case("lamstep","lam_step")
			read(key%value,*) lamStep(key%nr1)
