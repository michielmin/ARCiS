	subroutine Spec2RGB(F,lam,nlam,R,G,B)
	IMPLICIT NONE
	integer nlam
	real*8 F(nlam),lam(nlam),R,G,B,x,y,z
	
	call SPECTOXYZ(F,lam,nlam,x,y,z)
	R=x
	G=y
	B=z
	call XYZTORGB(x,y,z,R,G,B)
...
c ****************************************************************************
c
c      XYZ VALUES FROM SPECTRUM
c
	SUBROUTINE SPECTOXYZ(F_in,lam_in,nlam_in,X,Y,Z)
	IMPLICIT NONE
	integer nlam,i,j,k,nlam_in
	real*8 lam_in(nlam_in),F_in(nlam_in),X,Y,Z
	parameter(nlam=81)
	real*8 lam(nlam),F(nlam),XX,YY,ZZ
	real*8 WXYZ(3,nlam)
c
c      CIE Color Matching Functions (x_bar,y_bar,z_bar)
c      for wavelenghts in 5 nm increments from 380 nm to 780 nm.
c
	DATA ((WXYZ(I,J),I=1,3),J=1,nlam)/
...
c
	...
	ZZ=0.D0
	DO K=1,nlam
		lam(k)=380.D0+REAL(K-1)*5.D0
	ENDDO
	lam=lam/1d7
	call regridarray(lam_in,F_in,nlam_in,lam,F,nlam)
	DO k=1,nlam
		XX=XX+F(k)*WXYZ(1,K)
		YY=YY+F(k)*WXYZ(2,K)
		ZZ=ZZ+F(k)*WXYZ(3,K)
	...
	Y=YY
	Z=ZZ
c	X=XX/DMAX1(XX,YY,ZZ) 
c	Y=YY/DMAX1(XX,YY,ZZ)
c	Z=ZZ/DMAX1(XX,YY,ZZ)
	RETURN
	END
c
c*********************************************************************
c
	SUBROUTINE XYZTORGB(xc,yc,zc,R,G,B)
	IMPLICIT NONE
	real*8 xr,yr,zr,xg,yg,zg,xb,yb,zb,xc,yc,zc,R,G,B,xw,yw,zw
