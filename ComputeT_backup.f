	...
	allocate(Hstar_nu(nr,nlam,ng))
	allocate(Jstar_nu(nr,nlam,ng))
	...
	do ir=1,nr
		Hstar(ir)=0d0
		Hstar_nu(ir,1:nlam,1:ng)=0d0
		Jstar_nu(ir,1:nlam,1:ng)=0d0
		Cjstar(ir)=0d0
		do ilam=1,nlam-1
			do ig=1,ng
				tautot=0d0
				...
				Hstar_nu(ir,ilam,ig)=-must*contr
				Hstar(ir)=Hstar(ir)-must*dfreq(ilam)*wgg(ig)*contr
				Jstar_nu(ir,ilam,ig)=contr
...
18    continue
	...
	integer ii(3),j
	real*8 cM(3,3),cMLU(3,3),tauR(0:nr+2),tauR0(0:nr+2),tau1,tau2,ee0,ee1,ee2
		...
		do ig=1,ng
			...
			do ir=1,nr
				...
				do iscat=1,nscat
					...
					stopscat=.true.
					do jr=1,nr
						if(abs((Itot(jr)-Si(jr))/(Itot(jr)+Si(jr))).gt.1d-1) stopscat=.false.
					enddo
					Si=Itot
					if(stopscat) exit
				enddo
				do inu=1,nnu
					...
					enddo
					Ih(nr)=-(Ij(nr-1)-Ij(nr))/(tauR(nr-1)-tauR(nr))
					IntHnu(ilam,1:nr,ir)=IntHnu(ilam,1:nr,ir)-nu*wgg(ig)*dfreq(ilam)*Ih(1:nr)/real(nnu)
				enddo
			enddo
		enddo
	enddo
