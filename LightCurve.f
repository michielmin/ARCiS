...
c	open(unit=30,file='LDcoeff.dat',RECL=6000)
c	do ilam=1,nlam-1
...
c			open(unit=20,file=trim(outputdir) // "lightcurve" // trim(int2string(iobs,'(i0.4)')) // "_"  // trim(int2string(ilam,'(i0.4)')) // ".dat",RECL=6000)
...
c	enddo
	...
	open(unit=20,file=trim(outputdir) // "lightcurve.dat",RECL=6000)
	do j=1,nLightCurve
