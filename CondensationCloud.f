	...
	integer,allocatable :: IWORK(:),ixv(:,:),ixc(:,:),iVL(:,:),ixn(:),icryst(:),iamorph(:),inuc(:),ifit(:)
	...
	integer INCFD,IERR
	logical SKIP,liq
	real*8 time,kp,Otot(nr),Ctot(nr),Ntot(nr),compGibbs
	integer itime
...
1			continue
	...
	if(Cloud(ii)%computeJn) then
		fscale=1d-8
		pscale=(1d0/fscale)**(1d0/min(real(niter/2),100.0))
		f=0.95
		nfscale=0
	else
		fscale=1d0
